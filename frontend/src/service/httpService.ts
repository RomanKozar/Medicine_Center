...
class HttpService {
	// ➡️ ДОДАНО: Приватний метод для отримання токена
	private getAuthToken(): string | null {
		// Ваша логіка зберігання використовує лише localStorage,
		// але для надійності перевіряємо обидва та додаємо логування
		const token =
			localStorage.getItem('token') || sessionStorage.getItem('token')

		// Якщо ви використовуєте Next.js або SSR, може знадобитися додаткова перевірка,
		// але для client-side коду цього достатньо.
	...
	private getAuthHeaders(): Record<string, string> {
		// ВИПРАВЛЕННЯ: Використовуємо новий метод
